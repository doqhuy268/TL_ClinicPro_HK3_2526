...
async function main() {
  ...
  // 3.3b. Seed BoothService: gán dịch vụ cho từng booth theo phòng (phòng có dịch vụ → booth có dịch vụ)
  const booths = await prisma.booth.findMany({
    where: { isActive: true, isDeleted: { not: true } },
    select: { id: true, roomId: true },
  });
  for (const booth of booths) {
    const roomServices = await prisma.clinicRoomService.findMany({
      where: { clinicRoomId: booth.roomId },
      select: { serviceId: true },
    });
    for (const rs of roomServices) {
      await prisma.boothService.upsert({
        where: {
          boothId_serviceId: { boothId: booth.id, serviceId: rs.serviceId },
        },
        create: {
          boothId: booth.id,
  ...
  console.log(`✓ BoothService: đã gán dịch vụ cho ${booths.length} booth`);
  ...
  // 3.y Tạo work session bao trùm cả ngày cho 1–2 bác sĩ demo
  // Giúp luôn có giờ khám để test (kể cả ngoài giờ hành chính)
  const allDayTodayStart = new Date();
  allDayTodayStart.setHours(0, 0, 0, 0);
  const allDayTodayEnd = new Date();
  allDayTodayEnd.setHours(23, 59, 59, 999);

  const allDayTomorrowStart = new Date(allDayTodayStart);
  allDayTomorrowStart.setDate(allDayTomorrowStart.getDate() + 1);
  const allDayTomorrowEnd = new Date(allDayTomorrowStart);
  allDayTomorrowEnd.setHours(23, 59, 59, 999);

  async function createAllDayWorkSessionsForDoctor(doctorId: string) {
    const ranges = [
      { start: allDayTodayStart, end: allDayTodayEnd },
      { start: allDayTomorrowStart, end: allDayTomorrowEnd },
    ];

    for (const range of ranges) {
      const existedAllDay = await prisma.workSession.findFirst({
        where: {
          doctorId,
      ...
      if (existedAllDay) continue;

      // Tìm một booth bất kỳ theo chuyên khoa bác sĩ (nếu có)
      ...
      // Gán vài dịch vụ thuộc chuyên khoa cho all-day session
      const specialtyServices = await prisma.service.findMany({
        where: { specialtyId: doctor.specialtyId },
        orderBy: { name: 'asc' },
        take: 3,
  ...
  // Áp dụng all-day cho bác sĩ mặc định (Nội tổng quát) và 1 bác sĩ da liễu (nếu có)
  const defaultDoctorForAllDay = await prisma.doctor.findFirst({
    where: { auth: { email: 'doctor@gmail.com' } },
    include: { auth: true },
  });
  if (defaultDoctorForAllDay) {
    await createAllDayWorkSessionsForDoctor(defaultDoctorForAllDay.id);
  }

  const dermatologyDoctorForAllDay = await prisma.doctor.findFirst({
    where: { auth: { email: 'doctor.dermatology@gmail.com' } },
    include: { auth: true },
  });
  if (dermatologyDoctorForAllDay) {
    await createAllDayWorkSessionsForDoctor(dermatologyDoctorForAllDay.id);
  }
  ...
  // 4. Tạo dữ liệu demo cho 1 vòng quy trình khám bệnh
  try {
    console.log('🔁 Seeding demo flow: appointment -> medical record -> prescription -> invoice');

    // Lấy patient, doctor, cashier đã seed sẵn
    const patientAuth = await prisma.auth.findUnique({
      where: { email: 'patient@gmail.com' },
    });
    const doctorAuth = await prisma.auth.findUnique({
      where: { email: 'doctor@gmail.com' },
    });
    const cashier = await prisma.cashier.findFirst();

    if (!patientAuth || !doctorAuth || !cashier) {
      console.log(
        '⚠️  Demo flow seed skipped: missing patient/doctor/cashier base data',
      );
      return;
    }
    ...
    if (!doctor) {
      console.log('⚠️  Demo flow seed skipped: doctor entity not found');
      return;
    }
    ...
    if (!patient) {
      // Tạo Patient nếu chưa có (trong trường hợp seed thay đổi sau này)
    ...
    if (!profile) {
      profile = await prisma.patientProfile.create({
        data: {
          ...
          profileCode: 'PP-SEED-001',
          name: patientAuth.name,
          phone: patientAuth.phone ?? '0900000000',
          dateOfBirth: patientAuth.dateOfBirth,
          gender: patientAuth.gender,
          address: patientAuth.address,
          emergencyContact: {
            name: patientAuth.name,
            phone: patientAuth.phone ?? '0900000000',
            relationship: 'SELF',
    ...
    // Lấy 1 work session APPROVED trong tương lai cho bác sĩ
    const now = new Date();
    ...
    if (!workSession) {
      console.log(
        '⚠️  Demo flow seed skipped: no future APPROVED work session for doctor',
      );
      return;
    }

    const demoService =
      workSession.services.length > 0
        ? workSession.services[0].service
        : await prisma.service.findFirst({
            where: { specialtyId: doctor.specialtyId },
          });

    if (!demoService) {
      console.log('⚠️  Demo flow seed skipped: no service for doctor specialty');
      return;
    }

    // Tránh tạo trùng demo appointment
    const existingDemoAppointment = await prisma.appointment.findFirst({
      where: {
        appointmentCode: 'APT-DEMO-001',
      },
    });

    if (!existingDemoAppointment) {
      const appointmentDate = new Date(workSession.startTime);
      const appointmentDateStr =
        appointmentDate.toISOString().split('T')[0] ?? '';

      // Đơn giản: đặt lịch trong khoảng workSession (không cần chính xác slot logic)
      const startTimeStr = '09:00';
      const endTimeStr = '09:15';
      ...
      const appointment = await prisma.appointment.create({
        data: {
          ...
          bookerId: patientAuth.id,
          status: 'COMPLETED',
          date: new Date(appointmentDateStr),
      ...
      // Tạo medical record gắn với appointment
      ...
      const medicalRecord = await prisma.medicalRecord.create({
        data: {
          medicalRecordCode: 'MR-DEMO-001',
          templateId: template ? template.id : (await prisma.template.findFirst())!.id,
          patientProfileId: profile.id,
          doctorId: doctor.id,
          appointmentId: appointment.id,
          content: {
            chief_complaint: 'Đau đầu, mệt mỏi (demo)',
            diagnosis: 'Cảm cúm do virus (demo)',
            treatment_plan:
      ...
      // Tạo prescription + prescriptionService
      const prescription = await prisma.prescription.create({
        data: {
          prescriptionCode: 'PR-DEMO-001',
          doctorId: doctor.id,
          patientProfileId: profile.id,
          note: 'Phiếu chỉ định dịch vụ khám tổng quát demo',
          status: 'COMPLETED',
          medicalRecordId: medicalRecord.id,
        },
      });

      const prescriptionService = await prisma.prescriptionService.create({
        data: {
          ...
          appointmentId: appointment.id,
          status: 'COMPLETED',
          results: ['Kết quả demo: không phát hiện bất thường nghiêm trọng'],
          order: 1,
        },
      });

      // Tạo invoice + invoice detail + payment transaction
      const price = demoService.price ?? 200000;
      ...
      await prisma.invoiceDetail.create({
        data: {
          ...
          discountReason: null,
          prescriptionId: prescription.id,
          prescriptionServiceId: prescriptionService.id,
        },
      });
      ...
      await prisma.paymentTransaction.create({
        data: {
          ...
          currency: 'VND',
          status: 'SUCCEEDED',
          providerTransactionId: 'PAY-DEMO-001',
          orderCode: 'ORDER-DEMO-001',
          paymentUrl: null,
          qrCode: null,
          expiredAt: null,
          paidAt: new Date(),
          lastWebhookPayload: {} as Prisma.JsonObject,
      ...
      console.log('✅ Demo flow seeded: APT-DEMO-001 / MR-DEMO-001 / PR-DEMO-001 / INV-DEMO-001');
    } else {
      console.log('ℹ️  Demo flow already exists, skipping creation');
    }
  } catch (e) {
    console.error('⚠️  Error while seeding demo flow:', e);
  }
}

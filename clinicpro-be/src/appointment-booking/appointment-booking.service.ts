...
export class AppointmentBookingService {
    ...
    const doctors = await this.prisma.doctor.findMany({
      where: {
        ...
        workSessions: {
          some: {
            endTime: {
              gte: new Date(), // Lấy work sessions chưa kết thúc
            },
            status: {
  ...
  async getAvailableDoctors(
    ...
    const targetDate = vnDateToUtcMidnight(date);
  ...
  async getAvailableSlots(
    ...
    const workSessions = await this.prisma.workSession.findMany({
      where: {
        ...
        status: {
          in: ['PENDING', 'APPROVED', 'IN_PROGRESS'], // PENDING: cho phép đặt trước khi admin duyệt
        },
      },
      include: {
        services: {
    ...
    if (validWorkSessions.length === 0) {
      console.log('DEBUG getAvailableSlots: No valid work sessions', {
        doctorId,
        serviceId,
        date,
        workSessionsFound: workSessions.length,
        workSessionsWithService: workSessions.filter((ws) => ws.services.length > 0).length,
      });
      return {
        doctorId: doctor.id,
        doctorName: doctor.auth.name,
    ...
    for (const workSession of validWorkSessions) {
      ...
      while (currentTime < effectiveEnd) {
        ...
        const startTimeStr = formatTimeVn(slotStart);
        const endTimeStr = formatTimeVn(slotEnd);
  ...
  /**
   * Parse appointment time từ slot date và time string.
   * timeStr là giờ VN (HH:mm), chuyển sang UTC để so sánh với slot Date.
   */
  private parseAppointmentTimeFromSlot(slotDate: Date, timeStr: string): Date {
    const vnMoment = new Date(
      slotDate.getTime() + 7 * 60 * 60 * 1000,
    );
    const dateStr = `${vnMoment.getUTCFullYear()}-${String(vnMoment.getUTCMonth() + 1).padStart(2, '0')}-${String(vnMoment.getUTCDate()).padStart(2, '0')}`;
    return vnTimeToUtcDate(dateStr, timeStr);
  }
   ...
   * timeStr là giờ VN (HH:mm), chuyển sang UTC để so sánh với DB.
   */

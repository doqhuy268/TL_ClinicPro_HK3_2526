...
export class ClinicRoomService {
  ...
  async getPrescriptionServicesByClinicRoom(clinicRoomId: string) {
    ...
    // Find all supported service IDs to show WAITING patients who haven't been assigned yet
    const supportedServiceIds = new Set<string>();

    // 1. Services linked to clinic room
    const clinicRoomServices = await this.prisma.clinicRoomService.findMany({
      where: { clinicRoomId },
      select: { serviceId: true },
    });
    clinicRoomServices.forEach(cs => supportedServiceIds.add(cs.serviceId));

    // 2. Services linked to booths
    if (boothIds.length > 0) {
      const boothServices = await this.prisma.boothService.findMany({
        where: { boothId: { in: boothIds } },
        select: { serviceId: true },
      });
      boothServices.forEach(bs => supportedServiceIds.add(bs.serviceId));
    }

    // 3. Services linked to work sessions
    if (workSessionIds.length > 0) {
      const wsServices = await this.prisma.workSessionService.findMany({
        where: { workSessionId: { in: workSessionIds } },
        select: { serviceId: true },
      });
      wsServices.forEach(ws => supportedServiceIds.add(ws.serviceId));
    }
    ...
    if (supportedServiceIds.size > 0) {
      // Allow unassigned WAITING, PREPARING, SERVING, RETURNING patients to show up on this dashboard if the room supports their service
      orConditions.push({
        status: {
          in: [
            PrescriptionStatus.WAITING,
        ...
        serviceId: { in: Array.from(supportedServiceIds) },
        boothId: null,
        clinicRoomId: null,
      });
    }

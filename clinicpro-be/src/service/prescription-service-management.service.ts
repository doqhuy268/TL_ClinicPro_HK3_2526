...
export class PrescriptionServiceManagementService {
  ...
  async updateServiceResults(
    ...
    try {
      ...
      const prescriptionService =
        await this.prisma.prescriptionService.findUnique({
          ...
          include: {
            service: true,
            prescription: {
              select: { id: true, doctorId: true, belongsToServiceId: true },
            },
          },
        });
      ...
      // XỬ LÝ AUTO-RETURNING LUỒNG CẬN LÂM SÀNG:
      if (!shouldReschedule) {
        const pId = prescriptionService.prescription.id;
        // Kiểm tra xem tất cả các dịch vụ trong đơn chỉ định này đã hoàn tất chưa
        const siblingServices = await this.prisma.prescriptionService.findMany({
          where: { prescriptionId: pId },
          select: { status: true }
        });
        
        const allCompleted = siblingServices.every(
          s => s.status === PrescriptionStatus.COMPLETED || s.status === PrescriptionStatus.CANCELLED || s.status === PrescriptionStatus.SKIPPED
        );

        if (allCompleted) {
          const belongsToId = prescriptionService.prescription.belongsToServiceId;
          if (belongsToId) {
            // Lấy parent service (dịch vụ Bác Sĩ)
            const parentService = await this.prisma.prescriptionService.findUnique({
              where: { id: belongsToId },
              include: { prescription: { select: { doctorId: true } } }
            });
            
            if (parentService && parentService.status === PrescriptionStatus.WAITING_RESULT) {
              await this.prisma.prescriptionService.update({
                where: { id: belongsToId },
                data: { status: PrescriptionStatus.RETURNING }
              });
              
              this.logger.log(`Tự động auto-trigger Parent service ${belongsToId} thành RETURNING.`);
              
              // Trigger socket refresh for the original doctor
              if (this.prescriptionService && parentService.prescription?.doctorId) {
                try {
                  await this.prescriptionService.updateQueueInRedis(
                    parentService.prescription.doctorId,
                    'DOCTOR',
                  );
                } catch (redisError) {
                  this.logger.warn(`Failed to update Redis queue for parent doctor: ${(redisError as Error).message}`);
                }
              }
            }
          }

...
# I/O — hỗ trợ cả Kaggle và chạy local
BASE_DIR = Path(__file__).parent.parent
KAGGLE_PROC = Path("/kaggle/input/mimic-iv-proc-revita-2025-09-222")
LOCAL_PROC = BASE_DIR / "data" / "proc"
PROC = KAGGLE_PROC if KAGGLE_PROC.exists() else LOCAL_PROC
UNIFIED_PQT = PROC / "train_unified.parquet"  # unified training file (preferred)
UNIFIED_CSV = PROC / "train_unified.csv"      # fallback
WORK_DIR = Path("/kaggle/working") if Path("/kaggle/working").exists() else (BASE_DIR / "models")

# Label selection — giới hạn 50–100 bệnh phổ biến (phù hợp tiểu luận)
MIN_LABEL_FREQ = 100   # chỉ giữ ICD có tần suất >= 100 lần nhập viện
MAX_LABELS     = 100   # giới hạn top 100 bệnh thường gặp
...
# Tabular features (tuổi + giới tính) — ghép với TF-IDF
USE_TABULAR = True   # dùng age_norm + one-hot gender khi có cột age_at_admit, gender
...
# %% [5b] TABULAR FEATURES — age_norm + one-hot gender (M, F, U)
def encode_tabular(sub_df):
    """Chuẩn hóa tuổi (age/120) + one-hot giới tính [M, F, U] → shape (N, 4)"""
    age_col = "age_at_admit" if "age_at_admit" in sub_df.columns else "age"
    age_raw = sub_df[age_col].astype(float).fillna(50).clip(0, 120).values
    age_norm = (age_raw / 120.0).reshape(-1, 1).astype(np.float32)
    if "gender" in sub_df.columns:
        g = sub_df["gender"].astype(str).str.upper().fillna("U")
    else:
        g = pd.Series(["U"] * len(sub_df), index=sub_df.index)
    onehot = np.column_stack([
        (g == "M").astype(np.float32),
        (g == "F").astype(np.float32),
        ((g != "M") & (g != "F")).astype(np.float32),
    ])
    return np.hstack([age_norm, onehot])

USE_TABULAR_ACTUAL = USE_TABULAR and "age_at_admit" in df.columns and "gender" in df.columns
if USE_TABULAR_ACTUAL:
    Xt_tr = encode_tabular(train)
    Xt_va = encode_tabular(val)
    Xt_te = encode_tabular(test)
    Xtr = sparse.hstack([Xtr, sparse.csr_matrix(Xt_tr)], format="csr")
    Xva = sparse.hstack([Xva, sparse.csr_matrix(Xt_va)], format="csr")
    Xte = sparse.hstack([Xte, sparse.csr_matrix(Xt_te)], format="csr")
    print("Đã ghép tabular (age + gender) → X shape:", Xtr.shape)
else:
    USE_TABULAR_ACTUAL = False
    print("Bỏ qua tabular (không có age_at_admit/gender)")
...
joblib.dump({
    ...
    "cfg": {
        ...
        "USE_TABULAR": USE_TABULAR_ACTUAL,
    }
}, CKPT_MODEL, compress=3)
print("Saved model →", CKPT_MODEL)

# %% [10] INFERENCE helper
def predict_topk(texts, K=TOPK, ages=None, genders=None):
    """Dự đoán Top-K. Khi USE_TABULAR: cần ages, genders (list) hoặc dùng mặc định 50, 'U'."""
    ...
    if USE_TABULAR_ACTUAL:
        n = len(texts)
        ages = ages if ages is not None else [50] * n
        genders = genders if genders is not None else ["U"] * n
        sub = pd.DataFrame({"age_at_admit": ages, "gender": genders})
        Xt = encode_tabular(sub)
        X = sparse.hstack([X, sparse.csr_matrix(Xt)], format="csr")
...
for i in range(min(50, len(test))):
    srow = test.iloc[i]
    if USE_TABULAR_ACTUAL and "age_at_admit" in srow.index and "gender" in srow.index:
        topk = predict_topk([srow["text_clean"]], K=TOPK, ages=[srow["age_at_admit"]], genders=[str(srow["gender"])])[0]
    else:
